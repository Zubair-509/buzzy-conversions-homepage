...
class APIHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        elif parsed_path.path == '/api/convert/excel-to-pdf': # Added handler for Excel to PDF
    ...
    def handle_excel_to_pdf_conversion(self):
        ...
        try:
            ...
            if not excel_file_content or not excel_filename:
                ...
                self.send_header('Access-Control-Allow-Origin', '*')
                self.end_headers()
                response = {'error': 'No valid Excel file found in request'}
                self.wfile.write(json.dumps(response).encode())
                return
            ...
            def convert_async():
                try:
                    ...
                    conversion_storage[conversion_id].update({
                        ...
                        'error': None,
                        'metadata': result.get('metadata', {}),
                        'method': result.get('method', 'excel-to-pdf')
                    })
                    print(f"Stored result for {conversion_id} in conversion_storage")
            ...
            print(f"Error in Excel to PDF conversion: {e}")
            self.send_response(500)
            self.send_header('Content-type', 'application/json')
            self.send_header('Access-Control-Allow-Origin', '*')
            self.end_headers()
...
def run_server(port=8000):
    ...
    print("  POST /api/convert/excel-to-pdf - Excel to PDF conversion") # Added endpoint
